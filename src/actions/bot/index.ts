    ...
    if (chatbotDomain) {
      ...
      if (customerEmail) {
        ...
        if (checkIncludes(message)) {
          const link = `${process.env.HOST_URL}/portal/${id}/appointment/${checkCustomer?.customer[0].id}`;
        ...
        if (checkIncludePayment(message)) {
          const link = `${process.env.HOST_URL}/portal/${id}/payment/${checkCustomer?.customer[0].id}`;
          const generatedLink = extractURLfromString(link);
          if (generatedLink) {
            const response = {
              role: "assistant",
