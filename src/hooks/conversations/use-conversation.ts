...
import { getMonthName, pusherCilent } from "@/lib/utils";
import {
...
export const useChatTime = (createdAt: Date, roomId: string) => {
  ...
  const [messageSetAt, setMessageSetAt] = useState<string>();
  const [urgent, setUrgent] = useState<boolean>(false);

  const onsetMessageReciveDate = () => {
    const dt = new Date(createdAt);
    const curret = new Date();
    const currentDate = curret.getDate();
    ...
    if (difference <= 0) {
      setMessageSetAt(`${hr}:${min}${hr > 12 ? "PM" : "AM"}`);
      if (curret.getHours() - dt.getHours() < 2) {
        setUrgent(true);
      }
    } else {
      setMessageSetAt(`${date} ${getMonthName(month)}`);
    }
  };
...
export const useChatWindow = () => {
  ...
  //     pusherCilent.bind("realtime-mode", (data: any) => {
